...
class AdminController
{
	...
	public function admin_init(): void
	{
		...
		add_settings_field('redirect_to', 'Redirect to after clicking "Duplicate"', [$this, 'redirect_to_render'], 'duplicate_post', 'duplicate_post_main');
	}
	...
	public function redirect_to_render(): void
	{
		...
		<select name="duplicate_post_general[redirect_to]">
			<option value="<?= self::REDIRECT_TO_LIST ?>" <?= selected(self::REDIRECT_TO_LIST, $selectedValue) ?>>To the list of articles</option>
			<option value="<?= self::REDIRECT_TO_EDIT ?>" <?= selected(self::REDIRECT_TO_EDIT, $selectedValue) ?>>To duplicate item edit screen</option>
		</select>
	...
	public function duplicate_post()
	{
		...
		if (!$post) {
			wp_die("An error has occurred. The article $post_id is not found !", "Item not found!");
		}
